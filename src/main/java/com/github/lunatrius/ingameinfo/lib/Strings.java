package com.github.lunatrius.ingameinfo.lib;

@SuppressWarnings("HardCodedStringLiteral")
public class Strings {
	public static final String CONFIG_CATEGORY_GENERAL = "general";
	public static final String CONFIG_CATEGORY_ALIGNMENT = "alignment";

	public static final String CONFIG_FILENAME = "filename";
	public static final String CONFIG_FILENAME_DESC = "The configuration that that should be loaded on startup.";

	public static final String CONFIG_SHOWINCHAT = "showInChat";
	public static final String CONFIG_SHOWINCHAT_DESC = "Displays the overlay in chat.";

	public static final String CONFIG_ALIGNMENT_DESC = "Offsets for %s (X<space>Y).";
}
